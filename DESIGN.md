...
~\.rig\                          this machine. not committed anywhere
  rig.local.json                 machine paths, identities, secrets sources,
                                 ...
                                 not configured. Outside the tool tree because an
                                 installation owns that directory and replaces it on
                                 upgrade (decision 93); a copy left beside the tool by
                                 an older install is still read
...
```
rig init                    one-time setup; writes ~/.rig/rig.local.json, and rig.json in the data root
...
| 93 | **The machine file leaves the tool tree, because an installation owns the directory it runs from** (hugoforte/rig#16). `rig.local.json` sat beside `package.json`, which was safe only while every installation was a git clone that npm *linked* to. A package installed from the registry is a **copy** into a directory npm owns and replaces wholesale on every upgrade, so the first `npm i -g @hugoforte/rig@latest` would have taken the roots, the identities and the secrets sources with it — silently, and with no way to tell afterwards that there had ever been a config. So the default moves to `~/.rig/rig.local.json`: **one rule on every platform**, not `%APPDATA%` and `$XDG_CONFIG_HOME`, because `rig doctor` prints these paths and a second rule buys nothing a user of a single tool would notice. `RIG_LOCAL_CONFIG` is unchanged and still wins. The file beside the tool is still **read** when no home file exists, which is what lets every clone install made before this keep working with nothing done to it — but it is never **written**, so an installation cannot newly acquire a config in the one place an upgrade deletes. `doctor` notes the old location and names the new one; a note rather than a warning, because the old location still works and warnings here are for contradictions. This is the change #16 called its blocker and mis-recorded as expired: #5 closed, but #5 was "deepen roots and config" — it bought the *seam* (bin/roots.mjs is the sole reader and writer of the location, so this is one function) and never moved the file. **What it exposed:** the test suite's isolation was accidental. `test/harness.mjs` stripped every `RIG_` variable and redirected `GIT_CONFIG_GLOBAL`, but never pinned `$HOME` — it did not need to, because the machine file was looked for beside a tool that was already in a temp directory. With a home location, 116 tests read the developer's real data roots. Both harnesses now pin `USERPROFILE` and `HOME` for the same reason they pin `GIT_CONFIG_GLOBAL`, and the isolation is stated instead of inherited |
| 94 | **rig is published as `@hugoforte/rig`, and a package with no `.git` names itself from the version the release injected** (hugoforte/rig#16, ADR-0004's "issue #16 changes the sum"). The bare name `rig` has been taken on the npm registry since 2013, so **scoping is forced rather than stylistic**. The registry is also what makes version injection legal under ADR-0004: `npm version --no-git-tag-version` runs in the release runner and is never committed, so `main` gains no commit, no bypass actor is needed, and nothing measures itself behind because of a bump — every objection that ADR raised against a bumping bot simply does not apply. A `github:` install was rejected for the opposite reason: a bare one installs default-branch HEAD and is not a stable channel at all, and pinning `#semver:` resolves against the **tag's** `package.json`, so a real version has to reach that tree — by a commit on `main` (rejected twice by ADR-0004) or by a tag on an off-`main` commit, which stops `git describe` on `main` finding the tag and breaks the clone install's version instead. The trap moves and never closes. **`releaseMark` gains `package.json` as its second source, and git stays first**: a development checkout carries the placeholder *and* real history, so reading the file ahead of git would report a release the tree is not on. It is a **display** fact only — `FORMAT_STAMP` still reads nothing, which is the line ADR-0004 added to make the placeholder inert, and nothing here reaches it. **This settles the stable/edge question ADR-0004 deferred to #16, by dissolving it**: the two install shapes *are* the two channels — the registry is stable, the clone that `npm install -g` links is edge, and there is no `next` dist-tag, because publishing `main` on every merge would be a third channel nobody asked for and the clone already is the "run unreleased code" answer. "Everyone runs unreleased code" stops being true the moment users have `latest`; whoever is left on `main` is whoever is working on rig, which is who developer mode was always for. `files` is now explicit and `test/` and `.github/` stop shipping (79 files to 40), with `test/package.test.mjs` asserting what the packer actually does rather than what the field says — because the failure it guards is silent: **`bin/migrations/` decides `MAJOR`**, so a package that shipped without it would report a *lower* record format and migrate data roots backwards |
