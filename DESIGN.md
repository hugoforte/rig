...
| 77 | **A stage can carry its own ticket, and an open slice stops a close** — the two halves of a stage being a real unit of delivery rather than a line in a table. GitHub fires a closing keyword only for a pull request that merges into the **default branch**, and a stage's pull request merges into the work branch, so a slice's ticket can never close itself; rig is the only thing that can, and it does so **at `rig close`, with every other ticket** — one outward-facing moment rather than a new rule about when rig speaks. A slice that landed closes its ticket; one that did not is commented on and left open, on decision 65's reasoning that whether the problem is still worth solving is not rig's answer. An optional `tickets` on a `stages[]` entry is additive and survives a round trip through `saveWork`, so it costs no major (decision 74's test: nothing was ever stamped with a format this changes). The close blocker goes **in `workstate.mjs`**, not beside it: "is this work finished" is one table (decision 62), and the stack is passed in rather than derived there because `close` is the only caller that needs it — `list` runs over every work and a git and GitHub pass per stage per work is not what a listing is. The verdict stays about the work branch; a slice still up for review is simply more unfinished business. And **`rig close --force` records `forcedAt`**: forcing past the blockers is a decision, decision 64's rule is that a decision no lookup can recover is the one thing worth storing, and without it a work closed over an open pull request was indistinguishable from a rig bug — which is precisely what `contradictions` called it, telling you to file an issue about something you did on purpose. That rule is now `rig status`'s to report rather than `doctor`'s, because unlike its three neighbours it compares a record to **live state that keeps moving**, and `status` is the command that has already paid for the lookup. ❌ REJECTED: dropping the rule (it is true once the force is recorded); and having `doctor` fetch the facts (it runs over every work, which is the reason its own comment gave — a comment that described behaviour `status` did not yet have, and now does) |
| 78 | **A stage the branches could not place says so** (closes hugoforte/rig#89). `stageOrder`'s declaration-order fallback — the net decision 75 put under a stage somebody squashes anyway — was right and **invisible**: what came out was a plausible numbered list with nothing to say that the order had stopped being evidence, so two stages declared in one order and re-stacked in another were reported wrongly, confidently and silently. rig never merges anything, so it cannot stop a squash, and decision 75 states the convention as one rig relies on rather than enforces — which makes decision 66 the governing rule: **this is something to say, not to refuse**, and nothing new stops. `stageOrder` marks each stage `placed`, `stageState` carries it onto the stack entry, and `unplacedNote` turns it into one line wherever the order is shown — `rig stage`, the `rig plan` deploy-order table, the `rig pr` body, and `rig next`, whose "stage 2 of 5" is itself a claim about the order. Two judgements. **One note, not a mark per row**: a stack loses its footing all at once when the branch under it goes, and naming the branches in the note keeps the precision a column would have bought at a fraction of the noise. **The fact, never the guess**: "not placed by the branches" is what rig can see; "the branch below it was squashed" is a guess about somebody's merge button, and a plain rebase produces the same wreckage. The condition is **cut but unplaceable** (`started && !placed`) — an uncut stage is unplaced too, and it is ordinary and already reported as "not cut in any repo yet", so a new work with three declared stages grows no lines of doubt about an order nobody has had a chance to establish yet. The note renders **inside `stageTable`** rather than beside it at each call site, because `planIsStale` compares rendered regions: a note outside would leave a previously fresh rollout plan disagreeing with its stack with nothing able to see it, which is decision 73's read-back defeated by the thing meant to strengthen it. ❌ REJECTED: a `Placed` column (a column of "yes" for the ordinary case, to say something about the exception); naming the likely cause (a guess dressed as a finding); and refusing to render an order at all (decision 66, and the order shown is still the best answer available — a stage's own pull request answers its base live as soon as one exists) |
