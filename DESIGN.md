...
| 97 | **A stage's base is the stage below it, and the work branch only when there is none** (closes hugoforte/rig#142). `worktrees.chain()` picked the nearest ancestor out of one candidate set holding the work branch and the stages, and merging the stack down broke it: fast-forwarded to the top stage, the work branch *is* that stage's commit, so it was the nearest ancestor of the top stage and of nothing below it. `rig stage` then printed the top stage first, the rest in the declaration-order tail — a plausible numbered list, carried verbatim into the PR body `rig pr` writes and the deploy order `rig plan --refresh` renders. `adrift` could not catch it, correctly: the one wrong edge pointed inside the stack, and the stages left without one were an absence (decision 79). The same shape arrives through decision 75's own convention, when a stage lands with a merge commit and the next is cut from the work branch afterwards: the work branch was the new stage's base and the landed one had none, so the two printed upside down. **The fix is in the derivation, not the walk.** The stages keep saying which sits on which however far the work branch moves, so they answer first; the work branch is the base only of a stage with no stage below it, and then when the two are on one line of history in either direction — cut from it, or merged down into it. `stageOrder` is unchanged. A base therefore means *the branch below it in the stack* rather than *the commit it was cut from*, which differ only for a stage cut from a work branch that already held another stage — and nothing shows a git-derived base except through the order, where the stack reading is the one wanted. **Only a stage carrying this work's commits is read that way.** One nobody has committed on is still where it was cut, which is an ancestor of everything after it — the exact shape of a stage merged down first, with none of its commits — so read stages-first it became what every later stage sat on, and review found two stacks the first cut of this made worse: stages cut up front with only the first worked on, and an empty stage cut from the base branch. What a stage carries is measured from where the work branch meets the remote's base branch — their `merge-base` — and never from the mirror's own `refs/heads/<base>`, which is the first clone's and never moves, so every work after the first would have measured from a commit it did not start at; review caught that too, and the tests build every stack on a mirror whose main has moved on. An empty stage keeps the old reading. A measure that finds *every* stage empty tells none of them apart and is dropped, every stage then read stages-first: that is what the stack looks like once the work has merged into its base branch, where keeping it would bring the #142 order back. **Still ambiguous, and worse than before in one shape**: an empty stage cut from a work branch that already had commits, which the work branch has since moved past, sits on a commit of the work branch's own and is indistinguishable by ancestry from a stage merged down there — git does not record which branch a commit was made on — so a stage later cut from the work branch now reads as sitting on it, and the empty one prints first. It takes committing on the work branch directly, cutting a stage nobody then works on, and cutting another past it; the merged-down stack it is traded for is decision 75's own convention. and two landed siblings off the work branch are ranked by declaration order, because neither is an ancestor of the other. ❌ REJECTED: a note when the work branch and a stage share a commit (it makes a wrong list visibly partial where the right one is derivable); a second walk in `stageOrder` from the unplaced stages (it recovers the stages below and leaves the misplaced top one first, because the bad edge survives); storing declaration order (decision 70). **Left as they were**: a stage with a pull request reports the PR's base, which decision 75 makes the work branch; and a landed stage whose branch is gone leaves nothing to be an ancestor of the stage above — decision 79's known limitation | `test/worktrees.test.mjs` "a stack merged down into the work branch keeps its order, because the stages still say it"; `test/worktrees.test.mjs` "a stage cut from the work branch after the one below merged in sits on that one"; `test/worktrees.test.mjs` "stages cut but not yet committed on sit on the work branch, one on the next"; `test/worktrees.test.mjs` "a stage nobody has committed on is never what a stage with commits sits on"; `test/worktrees.test.mjs` "a stage cut from the base branch and not committed on stays out of the stack"; `test/worktrees.test.mjs` "a merged-down stack keeps its order after the work branch lands in the base branch"; `test/worktrees.test.mjs` "on a work branch nobody has committed on, a stage with commits still does not sit on an empty one" |
