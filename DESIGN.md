...
`talks_to` is the field that earns its keep: repo selection is graph traversal, and `rig impact
<repo>` is that traversal as a command.

`direction` is optional and names the answer rather than the arrow — `downstream` means "a
change here can break that one", `upstream` is the other way, `both` is both. Absent means
unstated, which is not the same as both ways. It is written from either end and read from
whichever end asks, so one entry saying `downstream` and the other saying `upstream` are one
claim agreed twice; two entries making *different* claims are a contradiction, reported and
...
**Enforced by** names the test that holds a decision to account — a path, then the test's own title — or `—` where nothing does. The empty cells are the point: they are the decisions this suite asserts and never checks, and that is the list to read when choosing what to test next. `test/design.test.mjs` resolves every reference and fails on one that has been renamed or deleted, because a column nobody reads back would assert enforcement that had quietly gone — which is the failure this column exists to catch, not to commit. Filling a cell is a mapping, never a promise: name the test that would fail if the decision stopped holding, and leave it empty rather than naming one that only passes nearby.
...
| 4 | Harness-owned bare mirrors, not worktrees off working clones | `test/worktrees.test.mjs` "the mirror is bare, rig-owned, and keeps remote branches out of its own refs" |
...
| 9 | Lazy mirrors, fetch on attach, no scheduled fetch | `test/worktrees.test.mjs` "every cut fetches, so a later worktree sees what the remote gained" |
| 10 | One shared branch name; base = per-repo remote HEAD | `test/worktrees.test.mjs` "the base is the repo's own remote HEAD, not a global default" |
...
| 23 | Work `AGENTS.md` is generated and regenerated, never edited | `test/base.test.mjs` "the generated AGENTS.md says where the branch lands now, not where it was cut from" |
...
| 38 | Every `gh` call lives in `bin/github.mjs` behind one interface; a second, in-memory adapter (`RIG_FAKE_GITHUB`) runs the ticket and PR paths under test | `test/github.test.mjs` "in-memory adapter: prForBranch finds the PR by branch" |
...
| 49 | The freshness line goes to **stderr** and is not gated on a TTY. An agent or CI job shelling out to rig is the audience that most needs telling, and stderr is what keeps it out of a pipe someone is reading an answer from | `test/freshness.test.mjs` "the ambient line is announced whether or not anyone is watching a terminal" |
...
| 54 | A check `doctor` cannot make on this machine is **dropped, never fatal**: the probe is chosen by platform (~~`Get-PSDrive` through PowerShell~~ `fs.statfsSync` on Windows, `df -Pk` on POSIX) and a missing or unreadable probe costs one line, not the verdict. Doctor is the command you run because something is already broken. **`Get-PSDrive` is superseded by `fs.statfsSync`** (hugoforte/rig#129): it answers without starting a process where PowerShell cost 202ms a call, and its label is the drive the work root resolves to, because statfs follows a junction and the path as written can name a different drive. It stays off POSIX because Linux counts statfs blocks in `f_frsize`, which Node does not report, and on a FUSE mount that reads as many times the space that is free | `test/doctor.test.mjs` "free space this machine could not measure costs one line, not the verdict (decision 54)"; `test/helpers.test.mjs` "freeSpace: a path the filesystem will not report on is null, so the check is dropped"; `test/helpers.test.mjs` "parseDf: output it cannot read is null, so the check is dropped rather than wrong"; `test/installation.test.mjs` "free space with nothing on PATH is answered on Windows, and never dies trying" |
...
| 92 | **The catalogue correction is offered by `rig next`, not asked for by `rig close`** (hugoforte/rig#120). Rule 4 asked for entries to be corrected “in passing”, which is discipline — the thing §8 designed around everywhere else, with entry-point capture for worktrees and ambient state for closing and nothing at all for knowledge. `close` looked like the moment, because the operator has just spent the work in those repos; it cannot be, and the reason is not any ordering inside close — the teardown does run before `commitAs`, which only sets the commit dispatch makes once the command returns. It is that **no rig command waits for a human**: close tears the worktrees down and exits, so a close that printed the request would be asking for work to be done on repos it had already deleted. `next` has every property wanted: it speaks only when asked, it only offers, and it runs through building, reviewing and landing — the span in which the repos are both in your head and on disk. `close` keeps a last call naming what is still a draft, and neither refuses: prose that stays wrong destroys nothing, which is what `verdict.blockers` exist for | `test/next.test.mjs` "a draft catalogue entry for an attached repo is offered for correction" |
| 93 | **The catalogue has a freshness of its own, measured against the mirror and reported without a verdict** (hugoforte/rig#119). `talks_to`, `setup` and `check` are facts about code that changes, and decision 3 let them through the durable-facts rule with no way to tell a written entry that was overtaken from one that still holds — `DRAFT: unreviewed` only ever said nobody had written it yet. So `doctor` counts the commits on a repo's default branch since its entry's own last commit here. Asked of the mirror rather than `gh`, which keeps every root check answerable without the network and bounds the measure to repos this machine has attached; a repo with no mirror answers null rather than zero. No threshold: rig cannot know which commits touched what an entry claims, so the count and the date are printed and the reader judges (CONTEXT.md's Freshness, generalised from the installation to anything committed that describes something else). Not folded into `bin/freshness.mjs`, which is about the tool checkout — two similar things are a coincidence | `test/doctor.test.mjs` "catalogue entries behind their repos warn and do not count, like drafts"; `test/attach.test.mjs` "a catalogue entry is reported as behind once the repo it describes has moved on" |
| 94 | **The catalogue’s graph leaves the renderer, and `talks_to` gains a direction** (hugoforte/rig#130, stage 1). `talks_to` was called the load-bearing field and the only thing that ever read it back was `rig demo`, which **drew** it — `buildGraph` sat in `bin/demo.mjs` serving a picture. §6 meanwhile makes the traversal a rule *for the agent* ("for every selected repo, check its neighbours and say why each is or isn’t in scope"), which is discipline standing where §8 puts a mechanism everywhere else. So the graph moves to `bin/catalog-graph.mjs` and `rig impact <repo>` asks it the question. **The point is not the command, it is that rule 4 finally has a reason behind it**: until something read an entry back, correcting one changed no outcome, and "correct it in passing" was a reminder rather than a payoff. **Direction names the answer, not the arrow.** `downstream` means "a change here can break that one"; `in`/`out` was rejected because on an event bus the calls and the data run opposite ways and neither reading is wrong. Absent is **unstated and not both ways** — decision 79’s rule that an unknown is not a fact — so `impact` prints the edge and declines to place it. Stored on either end and read relative to whichever end asks, because an edge is keyed by its two repos sorted (the merge `buildGraph` already did, on the reasoning that drawing two lines says the systems are more tangled than they are). **Two different claims about one relationship are a disagreement, reported and never resolved**, in decision 79’s shape: a contradiction, not an absence. `both` against `downstream` counts as one, deliberately — a precedence rule would absorb the disagreement quietly, in whichever direction the rule happened to favour. A value outside the vocabulary is **not** a direction and is still printed verbatim, because the only way anyone fixes it is by seeing the word they wrote. **Not a record-format change**, on decision 69’s precedent: one optional frontmatter key is knowledge about a repo, and decision 60’s majors are `work.json`’s. It cost the reader nothing either — `parseFrontmatter`’s nested branch already took a second key under a `talks_to` item, which is how `how:` works — so every existing entry stays valid and reads as unstated, and there is no backfill: §4.1 forbids the writing sprint, and the field arrives in the drafted template where rule 4 says the knowledge is cheap. **Freshness is joined on per edge, and only for the repos in the answer**: an entry the repo has moved on from asserts a weaker edge, and the measure costs a git spawn per entry with a mirror — `doctor` pays that across the whole catalogue because it reports on all of it, and a neighbourhood is a handful. **Two hops, fixed.** Three across a well-connected catalogue reaches most of an org and stops being an answer, and a depth flag is a knob with no value anyone could pick. **No composed direction at two hops**: A reaching B and B reaching C does not say A reaches C, because B may absorb it — so a two-hop neighbour carries its route and the far hop’s own direction, and rig draws no conclusion the catalogue did not state. ❌ REJECTED: keeping edges undirected and letting the prose carry it (true of the *arrow*, not of the *field* — a direction a human states is a durable fact on decision 3’s own terms); a `docs:` key pointing into a repo (unreadable during selection by decision 27, and the worktree is on disk after `attach`); a declared grouping above the repo, Dandy AI’s `product` (a group declared up front is a prediction, which is decision 63’s rule and why there is no `--track`) | `test/catalog-graph.test.mjs` "direction: two ends that disagree are a contradiction, and rig picks no winner"; `test/impact.test.mjs` "the two ends of one relationship are read as one claim, not two edges"; `test/attach.test.mjs` "rig impact weighs a neighbour by how far behind its entry is" |
| 95 | **The records are a second graph over the same repos, and where the two disagree is the output** (hugoforte/rig#130, stage 2). Decision 94 gave `talks_to` a consumer; this gives it a critic. Every work record already stores `repos[]`, so which repos have been attached to the same work is sitting in the data root and nothing read it. **The two graphs fail in opposite directions, which is why both are printed.** The declared one is hand-written and can be wrong, and decision 93 can only say how far behind an entry is, never what it got wrong; the observed one is a record rather than a derivation and cannot be wrong about what happened, but it can only ever see repos somebody has *already* worked on together — so it can never catch the fourth repo the first time, which is §6’s whole job. A pair the records keep making with no `talks_to` line between them is therefore the finding: evidence that an entry is missing an edge, and it names which entry. That is **rule 4 with a reason attached instead of a reminder** — the rule asks for corrections in passing and, until now, offered nothing that said *which* correction. **Not merged into the declared list.** Folding them together would put a claim nobody wrote in the same column as claims people did, and would lose the `declared` flag that is the entire point of showing them side by side. **Every work, not only the closed ones**: a work’s repo set is as true the day it is attached as the day it lands, and excluding open works hides the pairing a reader can still act on. Decision 60’s terminal-facts rule is about *PR timings*, which are not facts until they merge; `repos[]` never had that problem. **Pairs, never triples** — a work holding three repos is evidence about each of the three relationships, not about a three-way thing the catalogue cannot record. **Ordered by count**, which is the only ranking the records honestly support, and **no threshold**: decision 93’s rule, the number and the reader’s judgement. **The two offering commands take different graphs, and the split is how often each speaks.** `rig attach` runs once per repo, at the moment the repo set is live, and offers both — a co-attachment is worth raising when the question is open. `rig next` runs constantly and offers the declared graph only, because an observed pair is evidence about the *catalogue* and not about this work, and `next` has to stay quiet enough to be read (guardrail 1). `next` also goes silent from `reviewing` on: once a pull request is open, attaching a repo is a decision already taken, and naming it then is second-guessing rather than offering. **One offer for all of them, not one per repo** — a list that grows a line per neighbour is the to-do list `next` exists not to be. ❌ REJECTED: a minimum co-attachment count before a pair is worth reporting (a threshold nobody can choose a value for, which decision 93 already refused for the same reason); merging observed neighbours into `hop1` with a flag; auto-attaching anything — information, not automation | `test/catalog-graph.test.mjs` "impact: an observed pair with no talks_to line between them is the finding"; `test/impact.test.mjs` "a pair the records keep making is reported, with the works that made it"; `test/next.test.mjs` "nothing is offered once the work is up for review — attaching a repo then is a different decision"; `test/attach.test.mjs` "attaching a repo names what the catalogue says it talks to and is not attached" |
| 96 | **The demo page counts what the works produced, and the drawing points where the catalogue does** (hugoforte/rig#130, stage 3). The header counted inventory — repos, relationships, works, how many spanned more than one — and said nothing about what any of it cost or delivered, which is the half that makes the case to somebody deciding whether to adopt it. **Read out of the terminal PR facts** decision 60 already stores and `rig backfill` fills in behind: no `gh` call, so live and `--quick` agree by construction, and — the reason it may live in a committed page at all — a merge cannot un-merge, which is exactly the input test decision 91 set for this file and `rig dash` fails. **Landed means every one of its pull requests merged, never `closedAt`, and never an abandoned work**: the teardown ran for `--abandoned` too, so counting closes would report abandoned work as delivery, and counting a work on its *first* merge stops its clock early — the rule `rig dash` already counts by in `reduceWork`. **The cycle is the first commit in any repo to the last merge in any repo**, because a cross-repo work is not finished until its last repo is; taking the first merge would make the widest works look like the fastest, which inverts the thing the page is arguing. **Median, and every figure carries its n.** One migration that sat a month drags a mean far enough to be a lie, and `rig dash`’s rule that no statistic is printed without its n applies to the page that has an audience even more than to the one that does not. **The model keeps days to four places and the page chooses the unit.** Rounding to a tenth of a day in the model flattened every single-repo work to "0 days" — measured, on this data root: 51 landed works whose true median is 55 minutes — and a page that reads as broken argues against itself. **No claim rig cannot support.** Dandy AI’s landing page leads on "0 agent-caused rework in five months"; rig has no record of what the same work would have cost without it, so the page says so in a sentence rather than inventing a baseline. That is also the stronger argument, given the page’s own closing line is that if something looks wrong, the record is what is wrong. **An arrowhead where a direction is stated, a plain line where it is not**, and plain again where the two entries disagree — an unknown is not a fact and neither is a contradiction (decision 79). The drawing therefore shows where the catalogue is thin, which is decision 91’s argument for listing unlinked repos underneath it, applied to the edges instead of the nodes. Markers are SVG `<defs>`, so zero dependencies and `file://` both hold. **A marked end stops at the edge of the dot it points at**: the nodes are drawn over the edges with an opaque fill, and the first cut left each head at a node's centre, where it was painted over and the page showed plain lines — with every attribute test passing, which is why the tests now measure where the tip lands. ❌ REJECTED: a before-and-after; counting `closedAt` as delivery; rounding the cycle in the model; a mean | `test/demo.test.mjs` "outcome: a work lands when every one of its pull requests merged, not when it was closed"; `test/demo.test.mjs` "outcome: a work with one repo merged and another still out has not landed"; `test/demo.test.mjs` "outcome: the cycle runs from the first commit anywhere to the last merge anywhere"; `test/demo.test.mjs` "outcome: a cycle under a day is shown in hours, because zero days reads as a broken page"; `test/demo.test.mjs` "renderDemo: two entries that disagree are drawn plain rather than pointed one way"; `test/demo.test.mjs` "renderDemo: a downstream head is on the far end, and outside the dot it points at" |
| 97 | **The questions git answers out of its own files, rig reads out of its own files** (`bin/gitfs.mjs`). A git subprocess costs 55–65ms on Windows — `git --version`, which does nothing, is 55 of them — and the suite made 4818 of them, of which `rev-parse --show-toplevel` alone was 918. That question is "walk up from here until something is a repository", which is a handful of `stat` calls and a thousandth of the price; `symbolic-ref --short HEAD` is a file read or two; and `--absolute-git-dir` and `--git-common-dir` are the `.git` entry that walk already landed on and the `commondir` beside it. Replacing those four took the suite to 3457 spawns, a third of the saving being `git --version` probes nothing needs any more, because the commonest answer — this directory is not a checkout — now needs no git on PATH to give. **The rule is exactness, not speed**: everything the module answers is what git would have said, and everything else is `null`, which means *ask git* and never *no*. So `core.worktree` (git reports a directory no `.git` entry names), a repository format or extension it would have to interpret — a ref storage it cannot read among them — and `GIT_DIR` and its relatives in the environment each hand the question straight back — the failure being bought off is a fast wrong toplevel putting a work's records in the wrong data root, which is decision 80's guess worded as a fact with a stopwatch attached. **Three of git's behaviours had to be copied rather than guessed, and each was found by running git rather than by reading it**: at each level git looks for a `.git` inside a directory before it asks whether the directory is itself a bare repository, so a stray `git init --bare .` in a checkout's root still leaves a checkout — and a bare repository inside a checkout still answers for itself, because it has no `.git` and the walk reaches it before the checkout above it, which matters because rig's mirrors are bare repositories under a work root; an empty directory called `.git` is walked past; and a `.git` *file* naming nothing is fatal, which is the same fact told twice with opposite answers. **Two readings are deliberately better than git's rather than equal to it**, and both are `symbolic-ref --short` abbreviating for display: on a branch `rel` in a repository that also carries a tag `rel` it prints `heads/rel`, and for a HEAD outside `refs/heads/` it prints a name that is not a branch at all — rig puts that value in messages, compares it against a default branch and caches it as a branch name, so the ref is the answer and anything that is not a branch is null. ❌ NOT ATTEMPTED: resolving a sha, reading the index, and answering whether a ref exists — all three need `packed-refs`, and a ref store read half right is a wrong answer wearing the right shape; `rev-parse --verify --quiet` is 293 of the spawns that remain and is where that would pay. `git remote get-url origin` stays git's for a smaller version of the same reason: `url.<base>.insteadOf` rewrites it, and a config read that skipped the rewrite would name the wrong repo on exactly the machines that configure one. There is no injected runner, unlike decisions 57 and 80 — the seam those two take is a seam around a subprocess, and not having one is the point | `test/gitfs.test.mjs` "a bare repository has no work tree, and one inside a checkout still answers for itself"; `test/gitfs.test.mjs` "a checkout whose root is also shaped like a bare repository is still a checkout"; `test/checkouts.test.mjs` "placing a checkout costs no subprocess, and a directory that is none costs nothing at all" |
| 98 | **An invocation is a value, so a test can run one without a process** (hugoforte/rig#129). The suite drove the CLI as a subprocess ~400 times, and on the Windows runner a Node boot is 51ms before rig's module graph is parsed — about 20s of a 211s run, on four cores that buy almost nothing, spent to get one thing: an *environment* isolated from the machine. So `bin/rig.mjs` exports `run(argv, io)`, where `io` is that environment named — the installation the run is a run of, its cwd, its environment, its stdin, and a writer per stream — and the exit code is the return value. `main` is one line of it. **What made this small was that the seams already existed**: output was six sinks, failure was `die` → `RigError`, and every subprocess went through one runner — so the change is that each of those reads the run rather than the process. **The five module-level bindings become one per-run value**, `current`, together with the two memoised tracker adapters, and `run` puts back the one it displaced: what used to end when the process ended now ends when the invocation does. It is ambient rather than a parameter, and that is the trade — threading one argument through every function in a 3,300-line file would *be* the change, and what a second run needed was the state's lifetime, not the plumbing; the cost is one run at a time in a process, which is what the CLI has by construction and what `node --test` gives. **`onPath` is the exception and keys on PATH instead**: a run does not own PATH, the machine does, so one run's answer is the next one's for as long as they are handed the same one — which is also what stops a run given a crippled PATH being told what a whole one found. 368 of the 398 invocations moved; the 30 that stayed are the ones whose subject *is* the process, which is decision 81's rule applied to the adapter rather than to a check: `rig update` re-executing the tool that just arrived, the previous release on disk, and `rig check --run`, whose catalogue commands inherit rig's stdio and so reach an assertion only down a pipe — and, beside them, the CLI's own half of the seam: the stdin it reads, the folder it moves out of, and a reader that stops early. **Two behaviour changes, both narrowings.** Every subprocess rig starts now gets the run's environment rather than inheriting the process's, and the run's cwd when the run was handed one — a run handed none is the process's own, and its children inherit the process's cwd without anything reading it first, which is what lets a command that needs no directory run from a shell left standing in one that is gone, which is what `bin/checkouts.mjs`'s `opts.env` stopped being spawnSync-shaped for — it names additions, because replacing the environment would drop what an isolated run depends on. The same module takes the run's `env` as a thunk, for decision 97's `discover`: `GIT_DIR` and its four relations are what make `discover` hand a question back, so reading them off the process would be answering for a run that is not this one. And an unknown command sets `process.exitCode` where it used to `process.exit(1)`, which a caller in the same process could not have survived. ❌ REJECTED: `AsyncLocalStorage` for the ambient value — it buys concurrent runs in one process, which nothing wants and `node --test` does not do; and a fake filesystem or a fake git, which is where an in-process suite usually goes wrong — what moved here is the *process*, and every test still drives real git against real temporary repositories | `test/invocation.test.mjs` "two installations, one process, and neither run resolves the other's data root"; `test/invocation.test.mjs` "a tracker is a run's, so the next run reads the state file again rather than what it remembered"; `test/invocation.test.mjs` "the folder a run is standing in is the one it was handed, never the process's"; `test/invocation.test.mjs` "a run handed a crippled PATH is never told what a run with a whole one found"; `test/invocation.test.mjs` "a run handed no cwd asks the process for one only when a command needs it" |
| 99 | **On Windows rig starts git's own binary rather than the launcher PATH names, and hides the spawns of the one rig process that has no console** (hugoforte/rig#129). Git for Windows puts a 46KB launcher on PATH — `cmd\git.exe`, and `bin\git.exe` on the runner images — whose job is to start `mingw64\bin\git.exe`; and `windowsHide`, which is `CREATE_NO_WINDOW`, does not suppress a console but allocates a hidden one, a `conhost.exe` per spawn. Neither did any of the work, and together they made every git call three process creations where one would do: 2.2–2.4× the cost of the binary alone, measured interleaved over sixty calls. **The launcher is recognised, never assumed.** Only a `git` found the way a spawn finds it — one pair of quotes stripped, `git.com` then `git.exe`, a file and not a directory — in a `cmd` or `bin` directory with the real binary beside it is stepped past, because a `git` somebody put on PATH themselves is a program they chose; an entry the search cannot read the way the spawn would ends the search and keeps the launcher, since the next layout along would be somebody else's git. **With MSYSTEM set the launcher is kept**: it sets MSYSTEM and puts Git's own directories on PATH, and the binary does that for itself only when MSYSTEM is unset, so under an MSYS2 shell, a non-login bash or a user-wide MSYSTEM the binary started straight cannot find the `sh` every hook runs through. The answer is cached against PATH and MSYSTEM together, as `onPath` is against PATH (decision 98). **`windowsHide` stays on the freshness refresh alone**, the detached child with no console to hand on, where each git call would otherwise open a visible window. That rests on an assumption nothing here can check, so it is stated: every other rig process has a console for its children to inherit — a terminal, an editor's task, a scheduled task all give one — and a host that starts `node bin/rig.mjs` with DETACHED_PROCESS, or calls `run()` from a GUI process with no console, gets a window per git call | `test/helpers.test.mjs` "the Git for Windows launcher is stepped past, and nobody else's git is"; `test/helpers.test.mjs` "with MSYSTEM set the launcher is kept, because it is what gives git its own PATH"; `test/helpers.test.mjs` "a hook starts through the git a run is handed, with MSYSTEM set or not"; `test/helpers.test.mjs` "only the console-less run hides its spawns, because a hidden console is a whole process" |
