...
function parseArgs (argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i]
    // `--flag`, `--flag=value`, `--flag value`; a short `-m value` is its own long form.
...
function regenerate (cfg, work) {
  ...
  writeText(path.join(wd, WORK_FOLDER.claude), `See [AGENTS.md](./AGENTS.md).\n`)
  writeText(path.join(wd, WORK_FOLDER.marker, 'id'), work.id + '\n')
}

// ------------------------------------------------------ data root commits

// Every mutating command ends here: the whole data root (catalogue corrections made in
// passing included) is committed, then pushed if it has an upstream — event-based, no
// timer, no hook (DESIGN.md decision 20). Silent but announced: one line, never a
// prompt, and a push that fails warns rather than dies. Before pushing, others' commits
// are pulled with rebase; a conflict aborts the rebase and says so, so the data root is
// never left mid-rebase.
function commitDataRoot (message) {
  const root = dataRoot()
  // `git add -A` from inside the tool's tree would stage the tool itself: the leak
  // AGENTS.md warns about. Equal to it, or anywhere under it, is refused.
  ...
  if (git(root, 'rev-parse', '--is-inside-work-tree').out !== 'true') {
    say(C.dim(`· data root ${root} is not a git checkout — nothing committed`)); return
  }
  gitMust(root, 'add', '-A')
  const staged = git(root, 'diff', '--cached', '--quiet').code !== 0
  if (staged) gitMust(root, 'commit', '-q', '-m', message)
  const hash = () => git(root, 'rev-parse', '--short', 'HEAD').out
  if (git(root, 'rev-parse', '--abbrev-ref', '@{u}').code !== 0) {
    if (staged) ok(`data root: committed ${hash()} (no upstream — not pushed)`)
    else say(C.dim('· data root: nothing to commit'))
    return
  }
  // The tree is clean after `add -A` and the commit, so --autostash has nothing to do
  // today; the Direction asks for it, and it keeps this safe should that ever change.
  const pull = git(root, 'pull', '-q', '--rebase', '--autostash')
  if (pull.code !== 0) {
    const midRebase = ['rebase-merge', 'rebase-apply']
      .some(d => exists(path.resolve(root, git(root, 'rev-parse', '--git-path', d).out)))
    if (!midRebase) {
      warn(`data root: committed ${hash()} locally, but could not pull from origin (${pull.err.split('\n')[0] || 'no detail from git'}) — nothing pushed`)
      return
    }
    const abort = git(root, 'rebase', '--abort')
    if (abort.code !== 0) die(`data root: rebasing onto origin conflicted and the abort failed — sort ${root} out by hand (git status)\n${abort.err}`)
    warn(`data root: committed ${hash()} locally, but rebasing onto origin hit a conflict — rebase aborted, tree left clean; pull, resolve and push by hand in ${root}`)
    return
  }
  const ahead = Number(git(root, 'rev-list', '--count', '@{u}..HEAD').out) || 0
  if (!ahead) { say(C.dim('· data root: nothing to commit, nothing to push')); return }
  const push = git(root, 'push', '-q')
  if (push.code !== 0) { warn(`data root: committed ${hash()}, but the push failed (${push.err.split('\n')[0]}) — push it by hand`); return }
  ok(staged ? `data root: committed ${hash()} and pushed` : `data root: pushed ${hash()} (committed earlier, when its push failed)`)
}
...
cmds.new = async ({ flags, positional }) => {
  ...
  commitDataRoot(`rig new ${id}`)
}

// Takes the work itself, so `rig new --repos a,b` attaches to the record it just built
// instead of re-reading it once per repo.
...
cmds.ticket = ({ flags, positional }) => {
  ...
  commitDataRoot(`rig ticket ${id}: ${key}`)
}
...
cmds.attach = async ({ flags, positional }) => {
  ...
  await attachRepo(cfg, loadWork(cfg, id), name, { setup: !!flags.setup })
  commitDataRoot(`rig attach ${id}: ${name}`)
}
...
cmds.save = ({ flags }) => {
  ...
  const id = findWorkId(cfg, flags.work)
  const work = loadWork(cfg, id)
  const note = typeof flags.m === 'string' ? flags.m : ''
  if (flags.designed) {
  ...
  saveWork(cfg, work)
  commitDataRoot(`rig save ${id}${note ? `: ${note}` : ''}`)
}
...
cmds.detach = ({ flags, positional }) => {
  ...
  commitDataRoot(`rig detach ${id}: ${entry.repo}`)
}
...
cmds.close = ({ flags }) => {
  ...
  work.status = 'closed'
  saveWork(cfg, work)   // the folder's AGENTS.md reads as closed even if it outlives this
...
or take --work <id>. Every command that changes a work (new, ticket, attach,
detach, plan, save, close) ends by committing the whole data root, and pushing
it when it has an upstream.`)
}
